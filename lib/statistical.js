...
var exprEval = require('./expr_eval');
...
exports.COUNTIFS = function() {
  ...
  for (i = 0; i < args.length; i += 2) {
    ...
    for (var j = 0; j < range.length; j++) {
      var rangeJ = typeof range[j] !== 'string' ? range[j] : '"' + range[j] + '"';
      results[j] = results[j] && exprEval(rangeJ + criteria);
    }
  }
  var result = 0;
