...
function do_eval(node, variables) {
  if (node.type === 'BinaryExpression') {
    ...
    if (!binop) {
      return console.error('Unknown binary operator ' + node.operator);
    }
    return binop(do_eval(node.left), do_eval(node.right));
  } else if (node.type === 'UnaryExpression') {
    var unop = unops[node.operator];
    if (!unop) {
      return console.error('Unknown unary operator ' + node.operator);
    }
    return unop(do_eval(node.argument));
  } else if (node.type === 'LogicalExpression') {
    var logop = logops[node.operator];
    if (!logop) {
      return console.error('Unknown logical operator ' + node.operator);
    }
    ...
    console.error('Unsupported expr node', node);
  }
}
